...
class FNOModule(pl.LightningModule):
    def __init__(self, input_dim=1, output_dim=1, d_model=32, num_layers=6,
        ...
        in_channels = input_dim+domain_dim
        lifting_channel_ratio = 1 
        projection_channel_ratio = 1

        self.model = FNO(modes,in_channels,
                                    output_dim, 
                                    d_model,
                                    num_layers,
    ...
    def forward(self, x, coords_x):
        ...
        x = torch.cat((x,coords_x), dim=2) # (batch_size, 1 or 2+domain_dim), rows, cols)
    ...
    def training_step(self, batch, batch_idx):
        ...
        y_hat = self.forward(x, coords_x).permute(0,2,1)
