...
#from models.AFNO.AFNO_lightning import AFNOModule
from datasets import MetaDataModule

import pdb

device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
checkpoint_path = '../../paper_run_scripts/2d_experiments_ViTNO/lightning_logs/ku3v5arh/checkpoints/epoch=79-step=288000.ckpt'
...
api = wandb.Api()
run = api.run('edoardo-calvello/revision_experiments_Darcy_highres/ku3v5arh')
...
#valid_model_args = {key: value for key, value in config.items() if key in AFNOModule.__init__.__code__.co_varnames}
# Create the model using the same architecture
model = SimpleEncoderModule(**valid_model_args)
#model = FNOModule(**valid_model_args)
#model = AFNOModule(**valid_model_args)
#model.model.inp_shape = ((832,832))
...
model.model.set_im_size(416,32, test_sample_rate)
...
plt.savefig(f"test_error_plots_dt{test_sample_rate}_ViTNO_128_Darcy.png")
