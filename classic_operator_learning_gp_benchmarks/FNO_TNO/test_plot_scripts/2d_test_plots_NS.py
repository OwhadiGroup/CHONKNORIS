...
checkpoint_path = '../../paper_run_scripts/2d_experiments_FNO/lightning_logs/6g8pu65x/checkpoints/epoch=99-step=21300.ckpt'
checkpoint = torch.load(checkpoint_path, map_location=device)


api = wandb.Api()
run = api.run('edoardo-calvello/DeepNewtonOperator_Darcy/6g8pu65x')
...
#model.model.set_im_size(832,64,test_sample_rate)
...
data_sets = {
            ...
            (1, 3): np.log10(y_rel_diff_min+eps),
            (0, 0): x_input_median,
            (0, 1): y_x_diff_median,
            (0, 2): y_pred_x_diff_median,
            (0, 3): np.log10(y_rel_diff_median+eps)
        }
...
plt.savefig(f"test_error_plots_dt{test_sample_rate}_FANO_Reflect_128_NS.png")
